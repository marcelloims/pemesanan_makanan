...
class Model_admin extends CI_Model
{
	...
	{
		// $this->db->select('*', 'sum(tb_detail_orders.sub_total)');
		// $this->db->from('tb_orders');
		// $this->db->join('tb_detail_orders', 'tb_orders.no_invoice = tb_detail_orders.no_invoice');
		// $this->db->where('tanggal_invoice >=', $tanggal_awal);
		// $this->db->where('tanggal_invoice <=', $tanggal_akhir);
		// $this->db->group_by("tb_detail_orders.sub_total");
		// return $this->db->get();

		return $this->db->query('select tb_orders.*,sum(tb_detail_orders.sub_total) as total
		from tb_orders
			join tb_detail_orders
				on tb_detail_orders.no_invoice=tb_orders.no_invoice
				group by tb_detail_orders.no_invoice');
	}
}
